...
public class M_ham extends hamster {
	...
	public M_ham(String name, int love, int hungry, int clean, int water, int stamina, int power, int guts, int skill,
			int visual, int AP) {
		this.name = name;
		this.love = love;
		this.hungry = hungry;
	...
	public String[] getStatusRank() {
		...
		for (int i = 0; i < 4; i++) {
			if (getRank[i] < 200) {
				rank[i] = "い～";
			} else if (getRank[i] == 400) {
				rank[i] = "でぃ～";
			} else if (getRank[i] == 600) {
				rank[i] = "し～";
			} else if (getRank[i] == 800) {
				rank[i] = "び～";
			} else if (getRank[i] == 1000) {
				rank[i] = "え～";
			} else if (getRank[i] == 1200) {
				rank[i] = "えす";
			} else if (getRank[i] == 1500) {
				rank[i] = "さいきょう";
			}
		}
